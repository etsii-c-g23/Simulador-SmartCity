...
El WiFi Direct es un protocolo de conexión de dispositivos, el cual permite a dos entidades conectarse sin la necesidad de un router externo. Esto permite una mayor transmisión de datos y una menor latencia, mejorando la eficiencia. Su funcionamiento se basa en las siguientes 4 fases:
\begin{enumerate}
...
En primer lugar, el dispositivo legítimo (Semáforo) solicita acceso con sus credenciales reales (MAC terminada en \texttt{:66}) y una potencia de señal de -45 dBm. La Fig. \ref{fig:fase1} muestra cómo el sistema verifica la solicitud y concede el acceso ("ACCESO CONCEDIDO"), estableciendo una sesión activa.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Fase 1: Solicitud legítima del Semáforo.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
...
El atacante (Hugo el Hacker) intenta suplantar al semáforo copiando exactamente sus parámetros: misma dirección MAC y mismo RSSI (-45 dBm). Como se observa en la Fig. \ref{fig:fase2}, el protocolo detecta que esa MAC ya tiene una sesión activa registrada y bloquea inmediatamente el intento, protegiendo la integridad de la sesión en curso.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Fase 2: Intento con misma MAC y RSSI.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
...
El atacante modifica su estrategia: mantiene la MAC suplantada pero altera la potencia de emisión a -67 dBm, intentando simular un cambio de posición o fluctuación. La Fig. \ref{fig:fase3} evidencia que el sistema no se deja engañar; al persistir el uso de una MAC ya registrada en el sistema, la solicitud sigue siendo rechazada ("BLOQUEADO") independientemente del cambio en la señal.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Fase 3: Misma MAC con RSSI modificado.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
...
En esta fase, el atacante cambia su identidad (MAC nueva terminada en \texttt{:FF}) pero vuelve a emitir con la potencia original del semáforo (-45 dBm). El algoritmo de filtrado contextual identifica una anomalía física: es improbable que dos dispositivos distintos ocupen exactamente el mismo espacio radioeléctrico (mismo RSSI exacto). Por tanto, como muestra la Fig. \ref{fig:fase4}, se bloquea la solicitud por "Distinta MAC en misma ubicación".
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Fase 4: Nueva MAC con RSSI sospechoso.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{img/escenario2_logs4.png}
        \caption{Resultado: Bloqueo por anomalía física.}
    \end{minipage}
    \label{fig:fase4}
\end{figure}

Finalmente, se simula una conexión válida donde cambian tanto la identidad como el contexto. El dispositivo presenta una MAC nueva y un RSSI diferente (-19 dBm), lo cual es coherente con un nuevo nodo en la red. La Fig. \ref{fig:fase5} confirma que el sistema valida estos parámetros al no encontrar conflictos, registrando exitosamente el "NUEVO DISPOSITIVO".
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Fase 5: Nueva MAC y nuevo RSSI.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
